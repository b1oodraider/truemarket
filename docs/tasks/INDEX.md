...
| [TASK-096](./TASK-096.md) | Извлечь shared security (JWT-аутентификация) в `common` | 🟢 DONE | Rule-of-Three триггер сработал (catalog/TASK-109 — 2-й потребитель). JWT-фильтр + AuthenticatedUser + RFC7807 entry-points → `common.security` (shared OPEN-модуль: `@Modulithic(sharedModules="common")` + `type=OPEN`). DIP: порт `AccessTokenAuthenticator` (common) ← адаптер в auth; common НЕ зависит от auth. Поведение auth не изменилось. XFF-резолвер пока в auth (вынести при следующем потребителе). |
